 ...
 */
export function writeConfig(config, configPath, options) {
    const pathToUse = configPath || getConfigPath(false);
...
export function mergeConfigs(...configs) {
    const result = { $schema: CONFIG_SCHEMA };
    for (const config of configs) {
        ...
        if (config.compaction)
            result.compaction = { ...result.compaction, ...config.compaction };
        if (config.watcher)
            result.watcher = { ...result.watcher, ...config.watcher };
        if (config.share)
            result.share = config.share;
        if (config.snapshot !== undefined)
            result.snapshot = config.snapshot;
 ...
 */
export function addProviderToConfig(providerId, providerConfig, configPath, options) {
 ...
 */
export function removeProviderFromConfig(providerId, configPath, options) {
    const config = readConfig(configPath);
 ...
 */
export function addMCPServerToConfig(serverId, serverConfig, configPath, options) {
    const config = readConfig(configPath);
    if (!config.mcp) {
        config.mcp = {};
    }
 ...
 */
export function removeMCPServerFromConfig(serverId, configPath, options) {
    const config = readConfig(configPath);
 ...
 * Set model mặc định
 */
export function setDefaultModel(model, configPath, options) {
    const config = readConfig(configPath);
    config.model = model;
    writeConfig(config, configPath, options);
}
 ...
 */
export function initConfig(configPath, options) {
    const defaultConfig = {
