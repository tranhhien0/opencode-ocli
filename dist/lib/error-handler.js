 ...
 */
export function createOCXError(message, type = 'unknown', suggestion, details) {
...
export function classifyError(error) {
    ...
    if (lowerMessage.includes('network') ||
        ...
        return createOCXError(message, 'network', 'Lỗi kết nối mạng. Vui lòng kiểm tra internet và thử lại.\n' +
    ...
    if (lowerMessage.includes('permission denied') ||
        ...
        return createOCXError(message, 'permission', 'Lỗi phân quyền. Thử chạy lại với sudo (nếu cần) hoặc kiểm tra quyền truy cập file.\n' +
    ...
    if (lowerMessage.includes('not found') ||
        ...
        return createOCXError(message, 'not_found', 'Không tìm thấy resource. Kiểm tra lại tên/id đã nhập.\n' +
    ...
    if (lowerMessage.includes('invalid json') ||
        ...
        return createOCXError(message, 'invalid_config', 'Config file bị lỗi định dạng. Kiểm tra lại file opencode.json.\n' +
    ...
    if (lowerMessage.includes('401') ||
        ...
        return createOCXError(message, 'auth_failed', 'Lỗi xác thực. Kiểm tra API key hoặc thực hiện auth lại.\n' +
    ...
    if (lowerMessage.includes('timeout') ||
        ...
        return createOCXError(message, 'timeout', 'Yêu cầu hết thời gian chờ. Kiểm tra kết nối mạng hoặc thử lại sau.\n' +
            '  Có thể do server đang quá tải hoặc kết nối chậm.');
    }
    // Default: unknown error
    return createOCXError(message, 'unknown', 'Đã xảy ra lỗi không xác định. Thử chạy với --verbose để xem chi tiết.');
}
/**
