...
import { parseList } from '../lib/env.js';
...
provider.command('list')
    ...
    try {
        ...
        const result = {
            ...
            available_not_authed: knownProviders.filter(p => !authedProviders.includes(p) && !configProviders.includes(p))
        };
        if (options.json) {
...
provider.command('add')
    ...
    try {
        ...
        if (!providerId || !providerType) {
            throw createOCXError('Thiếu provider ID hoặc type', 'invalid_input', 'Vui lòng cung cấp cả --type và --id.\n' +
