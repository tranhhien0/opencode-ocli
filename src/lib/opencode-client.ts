...
export class OpenCodeClient {
  ...
  async listAuthProviders(): Promise<AuthProvider[]> {
    ...
    try {
      ...
      if (Array.isArray(data)) {
        return data.map((item: unknown) => {
          if (typeof item === 'string') return { id: item, authenticated: true };
          if (item && typeof item === 'object' && 'id' in item) {
            return item as AuthProvider;
          }
   ...
   */
  async installPlugin(name: string, options: PluginOptions = {}): Promise<void> {
   ...
   */
  async exportSession(id: string | null, options: SessionExportOptions = {}): Promise<string> {
      ...
      proc.on('error', (err) => {
        ...
        if (err.message.includes('ENOENT')) {
          reject(new OpenCodeError(
            ...
            { suggestion: 'Run: npm install -g opencode' }
          ));
        } else {
